...
async fn filter_task_by_day_should_succeed() {
    ...
    let result = test_bulder.services.task_service.filter_by_day(Utc::now()).await;
    assert!(result.is_ok());

    let yesterday = Utc::now().checked_sub_days(Days::new(1)).unwrap();
    let result = test_bulder.services.task_service.filter_by_day(yesterday).await;
    ...
    let result = test_bulder.services.task_service.filter_by_day(tomorrow).await;
